			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title">Manage User's Songs</h2>
...
<?php
 $sql = "SELECT tblsongs.SongTitle,tblsongs.status,tblsongs.aid,tblsongs.File,tblcat.catname,tblcat.cid,tblsongs.Price,tblsongs.songid from tblsongs join tblcat on tblcat.cid=tblsongs.cid where userid!='0'";
$query = $dbh -> prepare($sql);
$query->execute();
...
{				?>	
	...
	<audio controls><source src="mp3/<?php echo htmlentities($result->File);?>" class="img-responsive" type="audio/mpeg"/> </audio> 
		...
		<td>
		<a href="edit-song.php?id=<?php echo $result->songid;?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;
<a href="usersongs.php?del=<?php echo $result->songid;?>" onclick="return confirm('Do you want to delete');"><i class="fa fa-close"></i></a>

<?php if($result->status=="" || $result->status==0)
{
	?>										
	<a href="usersongs.php?aeid=<?php echo htmlentities($result->songid);?>" onclick="return confirm('Do you really want to Active')"> Active</a>
<?php }
 else {?>
			
<a href="usersongs.php?eid=<?php echo htmlentities($result->songid);?>" onclick="return confirm('Do you really want to Inactive')"> Inactive</a>
